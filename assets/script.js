document.addEventListener('DOMContentLoaded', function() {
    ...
    let score = 0;
    const chemicalRecipes = {
    'H2O': ['H', 'H', 'O'],         // Вода
    'CO2': ['C', 'O', 'O'],         // Углекислый газ
    'NH3': ['N', 'H', 'H', 'H'],    // Аммиак
    'NaCl': ['Na', 'Cl'],           // Поваренная соль
    'H2SO4': ['H', 'S', 'O', 'O', 'O', 'O'] // Серная кислота
};
  ...
  initGame();
    ...
    // Инициализация игры
    // Объект с рецептами веществ (добавить в начало скрипта)


// Инициализация игры (заменить текущую функцию)
function initGame() {
    const gameArea = document.getElementById('game-area');
    gameArea.innerHTML = '';
    selectedElements = [];
    ...
    // Создаем 12 случайных элементов
    const elements = ['H', 'O', 'C', 'N', 'Na', 'Cl', 'S', 'Fe'];
    for (let i = 0; i < 12; i++) {
        const randomElement = elements[Math.floor(Math.random() * elements.length)];
        const element = document.createElement('div');
        element.className = 'chemical-item';
        element.textContent = randomElement;
        element.setAttribute('data-element', randomElement);
        element.addEventListener('click', handleElementClick);
        gameArea.appendChild(element);
    }
}

// Обработчик клика по элементу (новая функция)
...
// Проверка комбинаций (новая функция)
function checkForReaction() {
    const resultElement = document.getElementById('compound-result');
    
    // Проверяем все рецепты
    for (const [compound, recipe] of Object.entries(chemicalRecipes)) {
        if (arraysMatch(selectedElements, recipe)) {
            score++;
            scoreElement.textContent = score;
            resultElement.textContent = `Получено: ${compound}!`;
            
            setTimeout(() => {
                initGame(); // Перезапуск игры после успеха
            }, 1500);
            return;
        }
    }
    
    // Если элементов слишком много без реакции
    if (selectedElements.length >= 5) {
        resultElement.textContent = 'Нет реакции. Попробуйте другую комбинацию!';
        setTimeout(() => {
            initGame();
        }, 1500);
    }
...
// Сравнение массивов (новая функция)
